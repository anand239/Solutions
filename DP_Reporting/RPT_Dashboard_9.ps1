...
Function Get-calendarReport
{
    ...
    $Pivot = foreach($UniqueClientGroup in $UniqueClientGroups)
    {
        $OBJ = @(
            @{Name = 'Account'; Expression = { $UniqueClientGroup.group | Select-Object -ExpandProperty Account -Unique}}
            @{Name = 'BkpApp'; Expression = { $UniqueClientGroup.group | Select-Object -ExpandProperty BkpApp -Unique}}
            @{Name = 'BackupServer'; Expression = { $UniqueClientGroup.group | Select-Object -ExpandProperty BackupServer -Unique}}
            @{Name = 'Clientname'; Expression = { $UniqueClientGroup.group | Select-Object -ExpandProperty Clientname -Unique}}
            @{Name = 'Specification'; Expression = { $UniqueClientGroup.group | Select-Object -ExpandProperty Specification -Unique}}
            @{Name = 'Object Type'; Expression = { $UniqueClientGroup.group | Select-Object -ExpandProperty 'Object Type' -Unique}}
            @{Name = 'Mode'; Expression = { $UniqueClientGroup.group | Select-Object -ExpandProperty Mode -Unique}}
            foreach($Uniquedate in $Uniquedates)
            {
                @{
                    Name = "$Uniquedate"
                    Expression = {@($UniqueClientGroup.group |
                        Where-Object date -eq $Uniquedate |
                        Select-Object -ExpandProperty "$ReportType")[0]}.GetNewClosure()
                }
            }
        )
        $UniqueClientGroup | Select-Object $OBJ
    }
    $Pivot
}
...
Function Get-BSRSummaryReport
{
    ...
    $UniqueClientGroups = $InputObject | Group-Object Account,BkpApp,BackupServer
    ...
    $Pivot = foreach($UniqueClientGroup in $UniqueClientGroups)
    {
        $OBJ = @(
            ...
            foreach($Uniquedate in $Uniquedates)
            {
                $SuccessfulObjCount = ""
                $TotalObjCount = ""
                $unq = $UniqueClientGroup.group | Where-Object date -eq $Uniquedate
                if($unq)
                {
                    foreach($Obj in $Unq)
                    {
                ...
                @{
                    Name = "$Uniquedate"
                    Expression = {$BSRPercentage}.GetNewClosure()
                }
            }
        )
        $UniqueClientGroup | Select-Object $OBJ
    ...
    $Pivot = foreach($UniqueClientGroup in $UniqueClientGroups)
    {
        $OBJ = @(
            ...
            foreach($Uniquedate in $Uniquedates)
            {
                @{
                    Name = "$Uniquedate"
                    Expression = {[math]::Round((($UniqueClientGroup.group | Where-Object date -eq $Uniquedate | Measure-Object -Property "Size (GB)" -Sum).Sum),2)}.GetNewClosure()
                }
            }
        )
        $UniqueClientGroup | Select-Object $OBJ
...
Function Get-CountReport
{
    ...
    $UniqueClientGroups = $RawData | Group-Object Account,BkpApp,BackupServer
    ...
    $Pivot = foreach($UniqueClientGroup in $UniqueClientGroups)
    {
        $OBJ = @(
            ...
            foreach($Uniquedate in $Uniquedates)
            {
                @{
                    Name = "$Uniquedate"
                    Expression = {@($UniqueClientGroup.group | Where-Object date -eq $Uniquedate | select $CountReportType -Unique).Count}.GetNewClosure()
                }
            }
        )
        $UniqueClientGroup | Select-Object $OBJ
...
Function Get-JobCounteachClientReport
{
    ...
    $UniqueClientGroups = $RawData | Group-Object Account,BkpApp,BackupServer,Clientname,'Object Type'
        ...
        $OBJ = @(
            ...
            foreach($Uniquedate in $Uniquedates)
            {
                @{
                    Name = "$Uniquedate"
                    Expression = {
                    @($UniqueClientGroup.group | Where-Object date -eq $Uniquedate).Count}.GetNewClosure()
                }
            }
        )
        $UniqueClientGroup | Select-Object $OBJ
...
Function Get-CalendarHTML
{
    ...
    New-HTMLTab -Name "$Name" { 
        New-HTMLContent -HeaderText "$HeaderText" -HeaderBackGroundColor "#5F249F"{
            ...
            } -DisableSelect -DisableStateSave -ScrollX -ScrollSizeY 300 -FixedFooter -FreezeColumnsLeft 7  -FixedHeader
        } 
    }
}
...
Function Get-SummaryHTMLReport
{
    New-HTMLTab -Name 'Summary Report'{
        if(($config.GenerateReport.AccountLevelSummaryReport -eq "Yes") -or ($config.GenerateReport.BackupServerLevelSummaryReport -eq "Yes") -or ($config.GenerateReport.BSRSummaryReport -eq "Yes") -or ($config.GenerateReport.SizeSummaryReport -eq "Yes") )
        {
        New-HTMLTab -Name "Daily Summary"{
            if($config.GenerateReport.AccountLevelSummaryReport -eq "Yes")
            {
            ...
            if($config.GenerateReport.BackupServerLevelSummaryReport -eq "Yes")
            {
            ...
            if($config.GenerateReport.BSRSummaryReport -eq "Yes")
            {
                New-HTMLTab -Name "BSR Daily Summary"{
                    New-HTMLContent -HeaderText "BSR Daily Summary Report  ( % )" -HeaderBackGroundColor "#5F249F" -HeaderTextColor White{
                        New-HTMLTable -ArrayOfObjects $BSRSummaryReport -HideFooter  -DisableNewLine -DisableSelect -DisableStateSave -ScrollX -ScrollSizeY 300 -FixedFooter  -FreezeColumnsLeft 3 -Buttons copyHtml5, excelHtml5, csvHtml5, pageLength, searchBuilder
                    } 
                    New-HTMLSection -HeaderText "Charts" -HeaderBackGroundColor "#5F249F" -CanCollapse -HeaderTextColor White{
            ...
            if($config.GenerateReport.SizeSummaryReport -eq "Yes")
            {
                New-HTMLTab -Name "Size Daily Summary"{
                    New-HTMLContent -HeaderText "Size Daily Summary Report  ( GB )" -HeaderBackGroundColor "#5F249F" -HeaderTextColor White{
                        New-HTMLTable -ArrayOfObjects $SizeSummaryReport -HideFooter  -DisableNewLine -DisableSelect -DisableStateSave -ScrollX -ScrollSizeY 300 -FixedFooter  -FreezeColumnsLeft 3 -Buttons copyHtml5, excelHtml5, csvHtml5, pageLength, searchBuilder
                    }
                    New-HTMLSection -HeaderText "Charts" -HeaderBackGroundColor "#5F249F" -CanCollapse -HeaderTextColor White{
        ...
        if($config.GenerateReport.MonthlySummaryReport -eq "Yes")
        {
...
Function Get-CalendarHTMLReport
{
    New-HTMLTab -Name 'Calendar View Report' {
        if($config.GenerateReport.BSRReport -eq "Yes")
        {
            Get-CalendarHTML -InputObject $BSRReport -Name "BSR" -HeaderText "BSR Calendar Report ( % )"
        }
        if($config.GenerateReport.SizeReport -eq "Yes")
        {
            New-HTMLTab -Name 'Size'{
                New-HTMLContent -HeaderText 'Size Calendar Report ( GB )' -HeaderBackGroundColor "#5F249F"{
                    New-HTMLTable -ArrayOfObjects $SizeReport -HideFooter -DisableNewLine  {
                    } -DisableSelect -DisableStateSave -ScrollX -ScrollSizeY 300 -FixedFooter  -FreezeColumnsLeft 7 -Buttons copyHtml5, excelHtml5, csvHtml5, pageLength, searchBuilder -FixedHeader
                }
            }
        }
        if($config.GenerateReport.DurationReport -eq "Yes")
        {
        ...
        if(($config.GenerateReport.ClientCountReport -eq "Yes") -or ($config.GenerateReport.JobCountReport -eq "Yes") -or ($config.GenerateReport.JobCountEachClientReport -eq "Yes"))
        {
            New-HTMLTab -Name 'Count'{
                if($config.GenerateReport.ClientCountReport -eq "Yes")
                {
                    New-HTMLContent -HeaderText 'Client Count Report' -HeaderBackGroundColor "#5F249F" -CanCollapse -HeaderTextColor White{
                        ...
                        } -DisableSelect -DisableStateSave -ScrollX -ScrollSizeY 300 -FixedFooter  -FreezeColumnsLeft 3 -Buttons copyHtml5, excelHtml5, csvHtml5, pageLength, searchBuilder
                    }
                }
                if($config.GenerateReport.JobCountReport -eq "Yes")
                {
                ...
                if($config.GenerateReport.JobCountEachClientReport -eq "Yes")
                {
                    New-HTMLContent -HeaderText 'Job Count Report for each Client' -HeaderBackGroundColor "#5F249F" -CanCollapse -HeaderTextColor White{
                        ...
                        } -DisableSelect -DisableStateSave -ScrollX -ScrollSizeY 300 -FixedFooter  -FreezeColumnsLeft 5 -Buttons copyHtml5, excelHtml5, csvHtml5, pageLength, searchBuilder
                    }
                }
            }
        }
        if($config.GenerateReport.ContinuousFailures -eq "Yes")
        {
...
try
{
    Import-Module ".\Pswritehtml\PSWriteHTML.psd1"
}
catch
{
...
if($config)
{
    ...
    if($BSRRepFiles)
    {
        ...
        {
            $DeleteDays = (Get-Date).AddDays(-$config.deleteFilesOlderThanInDays).ToString("yyyy-MM-dd") -replace "/","-"
            ($BSRRepFilePaths | where{$_.date -lt $DeleteDays}).filepath | Remove-Item
            $BSRRepFilePaths = $BSRRepFilePaths | where{$_.date -ge $DeleteDays}
        }
        ...
        if($config.Reportdays)
        {
            ...
            elseif($config.Reportdays -like "*_*")
            {
                $ReportFiles = $BSRRepFilePaths | where{$_.YearMonth -eq $config.Reportdays}
            }
            else
            {
                $RequiredDate = $config.Reportdays + "_" + "Days"
                $ReportFiles = $BSRRepFilePaths | select -Last $config.Reportdays
            }
        }
        else
        {
        ...
        if(!($ReportFiles))
        {
            Write-Log -Path $Activitylog -Entry "No CSV files found for the given report days" -Type Warning -ShowOnConsole
            exit
        }
        ...
        {
            $UniqueClientGroups = $ConsolidatedBSRRep | Group-Object Account,BkpApp,Clientname,BackupServer,Specification,'Object Type',mode
            #$Uniquedates = ($ConsolidatedBSRRep| Sort-Object date -Descending | Select-Object date -Unique).date | where{$_}
            $Uniquedates = ($ConsolidatedBSRRep | select date -Unique | select @{label="date";Expression= {[datetime]::ParseExact($_.date,"dd-MMyyyy",$null)}} | Sort-Object date -Descending).date
            ...
            if($config.GenerateReport.SizeReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating Size Report" -Type Information -ShowOnConsole
                $SizeReport     = Get-CalendarReport -InputObject $UniqueClientGroups  -ReportType "Size (GB)"
            }

            if($config.GenerateReport.BSRReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating BSR Report" -Type Information -ShowOnConsole
                $BSRReport      = Get-CalendarReport -InputObject $UniqueClientGroups  -ReportType "Percentage"
            }

            if($config.GenerateReport.DurationReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating Duration Report" -Type Information -ShowOnConsole
                $DurationReport = Get-CalendarReport -InputObject $UniqueClientGroups  -ReportType "Duration (min)"
            }

            if($config.GenerateReport.ClientCountReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating Client Count Report" -Type Information -ShowOnConsole
                $UniqueClientCountReport  = Get-CountReport -RawData $ConsolidatedBSRRep -CountReportType "ClientName"
            }

            if($config.GenerateReport.JobCountReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating Job Count Report" -Type Information -ShowOnConsole
                $UniqueJobCountReport     = Get-CountReport -RawData $ConsolidatedBSRRep -CountReportType "Specification"
            }

            if($config.GenerateReport.JobCountEachClientReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating Job Count Each Client Report" -Type Information -ShowOnConsole
                $JobCounteachClientReport = Get-JobCounteachClientReport -RawData  $ConsolidatedBSRRep
            }

            if($config.GenerateReport.AccountLevelSummaryReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating Account Level Summary Report" -Type Information -ShowOnConsole
                $SummaryReport            = Get-SummaryReport         -InputObject $ConsolidatedBSRRep
            }

            if($config.GenerateReport.BackupServerLevelSummaryReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating BackupServer Level Summary Report" -Type Information -ShowOnConsole
                $DetailedSummaryReport    = Get-DetailedSummaryReport -InputObject $ConsolidatedBSRRep
            }

            if($config.GenerateReport.BSRSummaryReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating BSR Summary Report" -Type Information -ShowOnConsole
                $BSRSummaryReport         = Get-BSRSummaryReport  -InputObject $ConsolidatedBSRRep
            }

            if($config.GenerateReport.SizeSummaryReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating Size Summary Report" -Type Information -ShowOnConsole
                $SizeSummaryReport        = Get-SizeSummaryReport -InputObject $ConsolidatedBSRRep
            }

            if($config.GenerateReport.ContinousFailureReport -eq "Yes")
            {
                Write-Log -Path $Activitylog -Entry "Generating Continous Failure Report" -Type Information -ShowOnConsole
            ...
            Write-Log -Path $Activitylog -Entry "Generating HTML Report" -Type Information -ShowOnConsole
            if(($config.GenerateReport.AccountLevelSummaryReport -eq "Yes") -or ($config.GenerateReport.BackupServerLevelSummaryReport -eq "Yes") -or ($config.GenerateReport.BSRSummaryReport -eq "Yes") -or ($config.GenerateReport.SizeSummaryReport -eq "Yes") -or ($config.GenerateReport.MonthlySummaryReport -eq "Yes") )
            {
            New-HTML -TitleText 'Summary-BKP_Report' -FilePath "$Summary_BKPReportName"  {
            ...
            if(($config.GenerateReport.SizeReport -eq "Yes") -or ($config.GenerateReport.BSRReport -eq "Yes") -or ($config.GenerateReport.DurationReport -eq "Yes") -or ($config.GenerateReport.ClientCountReport -eq "Yes") -or ($config.GenerateReport.JobCountReport -eq "Yes") -or ($config.GenerateReport.JobCountEachClientReport -eq "Yes") -or ($config.GenerateReport.ContinousFailureReport -eq "Yes"))
            {
                New-HTML -TitleText 'Calender-BKP_Report' -FilePath "$Calendar_BKPReportName"  {
                    New-htmllogo -LeftLogoString "data:image/png;base64,"
                    New-HTMLTabOptions -SlimTabs -SelectorColor "#5F249F"
                    Get-CalendarHTMLReport
                }
            }
            New-HTML -TitleText 'Consolidated-BKP_Report' -FilePath "$Consolidated_BKPReportName"  {
                New-htmllogo -LeftLogoString "data:image/png;base64,"
                New-HTMLTabOptions -SlimTabs -SelectorColor "#5F249F"
                Get-ConsolidatedHTMLReport    
            }
            if(($config.GenerateReport.BSRReport -eq "Yes") -or ($config.GenerateReport.ContinousFailureReport -eq "Yes"))
            {
                New-HTML -TitleText 'Calendar-BKP_BSR-Report' -FilePath "$Calendar_BKP_BSRReportName"  {
                    New-htmllogo -LeftLogoString "data:image/png;base64,"
                    New-HTMLTabOptions -SlimTabs -SelectorColor "#5F249F"
                    New-HTMLTab -Name 'Calendar View Report' {
                        if($config.GenerateReport.BSRReport -eq "Yes")
            ...
            New-HTML -TitleText ' Master-BKP_Report' -FilePath "$Master_BKPReportName"  {
                New-htmllogo -LeftLogoString "data:image/png;base64,"
                New-HTMLTabOptions -SlimTabs -SelectorColor "#5F249F"
                if(($config.GenerateReport.AccountLevelSummaryReport -eq "Yes") -or ($config.GenerateReport.BackupServerLevelSummaryReport -eq "Yes") -or ($config.GenerateReport.BSRSummaryReport -eq "Yes") -or ($config.GenerateReport.SizeSummaryReport -eq "Yes") -or ($config.GenerateReport.MonthlySummaryReport -eq "Yes") )
                {
            ...
            {
                if(($config.SendReports.'Master-BKP_Report_All_html' -eq "Yes") -or ($config.SendReports.'Master-BKP_Report_YYYY_MM_html' -eq "Yes"))
                {
                ...
                if(($config.GenerateReport.AccountLevelSummaryReport -eq "Yes") -or ($config.GenerateReport.BackupServerLevelSummaryReport -eq "Yes") -or ($config.GenerateReport.BSRSummaryReport -eq "Yes") -or ($config.GenerateReport.SizeSummaryReport -eq "Yes") -or ($config.GenerateReport.MonthlySummaryReport -eq "Yes") )
                {
                    if(($config.SendReports.'Summary-BKP_Report_All_html' -eq "Yes") -or ($config.SendReports.'Summary-BKP_Report_YYYY_MM_html' -eq "Yes"))
                    {
                ...
                if(($config.GenerateReport.SizeReport -eq "Yes") -or ($config.GenerateReport.BSRReport -eq "Yes") -or ($config.GenerateReport.DurationReport -eq "Yes") -or ($config.GenerateReport.ClientCountReport -eq "Yes") -or ($config.GenerateReport.JobCountReport -eq "Yes") -or ($config.GenerateReport.JobCountEachClientReport -eq "Yes") -or ($config.GenerateReport.ContinousFailureReport -eq "Yes"))
                {
                    if(($config.SendReports.'Calendar-BKP_Report_All_html' -eq "Yes") -or ($config.SendReports.'Calendar-BKP_Report_YYYY_MM_html' -eq "Yes"))
                    {
                ...
                if(($config.GenerateReport.BSRReport -eq "Yes") -or ($config.GenerateReport.ContinousFailureReport -eq "Yes"))
                {
                    if(($config.SendReports.'Calendar-BKP_BSR_Report_All_html' -eq "Yes") -or ($config.SendReports.'Calendar-BKP_BSR_Report_YYYY_MM_html' -eq "Yes"))
                    {
                        $attachment = @($Calendar_BKP_BSRReportName)
                        Send-Mail -attachments $attachment
                    }
                }
                if(($config.SendReports.'Consolidated-BKP_Report_All_html' -eq "Yes") -or ($config.SendReports.'Consolidated-BKP_Report_YYYY_MM_html' -eq "Yes"))
                {
                    $attachment = @($Consolidated_BKPReportName)
                    Send-Mail -attachments $attachment
                }
                if(($config.SendReports.'Consolidated-BKP_Report_All_csv' -eq "Yes") -or ($config.SendReports.'Consolidated-BKP_Report_YYYY_MM_csv' -eq "Yes"))
                {
