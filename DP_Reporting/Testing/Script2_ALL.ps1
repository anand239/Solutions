...
Function Get-ContinuousFailures
{
    ...
    foreach($Line in $InputObject)
    {
        ...
        {
            if(($Line).$LatestDatesProperty -eq 100)
            {
                $Count++
            }
        }
...
if($config)
{
    ...
    if($DownloadPathFlag)
    {
        ...
        if($BSRRepFiles)
        {
            ...
            $SizeReport = @()
            $SizeReport = Get-CalendarReport -InputObject $UnqClients -RawData $ConsolidatedBSRRep -ReportType "Size (GB)"
            #########################################################################################################################################

            $UnqClients = Get-UniqueClients  -InputObject $UniqueClientGroups
            $BSRReport  = @()
            $BSRReport  = Get-CalendarReport -InputObject $UnqClients -RawData $ConsolidatedBSRRep -ReportType "Percentage"
            #########################################################################################################################################

            $UnqClients     = Get-UniqueClients  -InputObject $UniqueClientGroups
            $DurationReport = @()
            $DurationReport = Get-CalendarReport -InputObject $UnqClients -RawData $ConsolidatedBSRRep -ReportType "Duration (min)"
            #########################################################################################################################################
