    ...
    if($OsType)
    {
        ...
        if($OsType -eq "Windows")
        {
            ...
            for($i=1;$i -le $ReportDays ;$i++)
            {
                ...
                $BCRReportName = $config.Reportpath  + "\" + "OBJ_Reports"+ "\" +  $config.BackupApplication + "_" + $config.Account + "_" + $Backupdevice + "_" + "BCRRep" + "_" + "$ReportEndDate" + ".csv"
                ...
                if(!("# No sessions matching the search criteria found." -in $SessionDetailsOutput))
                {
                    ...
                    $BCRs | Export-Csv -Path $DPReportName -NoTypeInformation
            ...
            if($sshsessionId.connected -eq "True")
            {
                ...
                for($i=1;$i -le $ReportDays ;$i++)
                {
                    ...
                    if(!("# No sessions matching the search criteria found." -in $SessionDetailsOutput))
                    {
                        ...
                        $SessionList | Export-Csv -Path $DPReportName -NoTypeInformation
                        $BCRs | Export-Csv -Path $BCRReportName -NoTypeInformation
                        $SessionTypeisBackup = $SessionList | Where-Object {$_."Session Type" -eq "Backup"}
                        if($SessionTypeisBackup)
                        {
                            $BSRReport = Get-BSRReport -InputObject $SessionList
...
# 1. Mail Function added.
# 2. Added 4 functions - Get-DailySummary,Get-MonthlySummary and respective data.
# 3. $InputObject = $InputObject -replace "," in Get-SessionList
# 4. Get-SSHTrustedHost | where{$_.sshhost -eq "$BackupDevice"}| Remove-SSHTrustedHost
# 5. Added object report new function