...
Function Get-CalendarHTML
{
    ...
    New-HTMLTab -Name "$Name" { 
        New-HTMLContent -HeaderText "$HeaderText" -HeaderBackGroundColor "#5F249F"{
            New-HTMLTable -ArrayOfObjects $InputObject -HideFooter -DisableNewLine -AutoSize{
    ...
    if($BSRRepFiles)
    {
        ...
        if($ConsolidatedBSRRep)
        {
            ...
            if ($config.SendEmail -eq "yes")
            {
                ...
                foreach($Mailattachment in $attachment)
                {
                    Send-Mail -attachments $Mailattachment
                }
            }
        }
        else
