    ...
    if($OsType)
    {
        if($OsType -eq "Windows")
        {
            ...
            for($i=1;$i -le $ReportDays ;$i++)
            {
                ...
                $BSRReportName = $config.Reportpath + "\" +  $config.BackupApplication + "_" + $config.Account + "_" + $Backupdevice + "_" + "BSR-Size" + "_" + "$ReportEndDate" + ".csv"
                ...
                $AllGroups = $SessionList | Where-Object {$_."Session Type" -eq "Backup"} | Group-Object 'session type',Specification,Client,status,mode
                $Report = @()
                foreach($Group in $AllGroups)
                {
                    ...
                    "BackupApplication" = $MountPoints[0].BackupApplication
                ...
                $Report | Export-Csv -Path $BSRReportName -NoTypeInformation
            }
        }
        else
        {
