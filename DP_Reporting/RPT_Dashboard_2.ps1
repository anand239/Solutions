...
Function Get-SizeSummaryReport
{
    ...
    foreach($Uniquedate in $Uniquedates)
    {
        ...
        foreach($UniqueAccApp in $UniqueAccApps)
        {
            ...
            {
                $SumOfSize = ($unq | Measure-Object -Property "size (GB)" -Sum).Sum
                $UniqueAccApp."$Uniquedate" = $SumOfSize
            }
            else
            {
                $UniqueAccApp."$Uniquedate" = $Null
...
if($config)
{
    if(!(Test-Path 'Reports'))
    {
        try
        {
            New-Item -ItemType directory "Reports" -ErrorAction Stop | Out-Null
        }
        catch
        {
    ...
    if ($config.DownloadFromOutLook -eq "yes")
    {
        ...
        if($DownloadPathFlag)
        {
            ...
            #$BSRRepFiles = Get-ChildItem -Path "$($DownloadPath)\*" -Include "*.csv" | Where-Object{$_.Name -like "*_BSR-Size_*"}
    ...
    if($BSRRepFiles)
    {
        ...
        if($ConsolidatedBSRRep)
        {
            ...
            $ConsolidatedBSRReportName = $config.ReportPath + "\" + "Reports" + "\" + "Consolidated_BKP-Rep" + "_" + $Config.Reportdays + ".csv"
            $ConsolidatedBSRRep | Export-Csv -Path $ConsolidatedBSRReportName -NoTypeInformation
            $HTMLReportName = $config.ReportPath + "\" + "Reports" + "\" + "HTML" + "_" + $Config.Reportdays + ".html"
            ...
            New-HTML -TitleText 'Reporting' -FilePath $HTMLReportName -Show {
                ...
                New-HTMLTab -Name 'Summary Report'{
                    ...
                    New-HTMLTab -Name "Size Daily Summary"{
                        ...
                        New-HTMLSection -HeaderText "Charts" -HeaderBackGroundColor "#5F249F" -CanCollapse -HeaderTextColor White{
                            New-HTMLChart -Title 'Daily Size Summary Chart' -TitleAlignment center{
                                New-ChartAxisX -Name $BSRProperties
...
#1. Bsr files can be taken from multiple folders.