...
foreach($Server in $Servers)
{
    ...
    $Failed_Client_count = @($Client | Where-Object{$_.status -eq "failed"}| sort client -Unique).Count 
    ...
    $FailedJobs = $Client | Where-Object{$_.status -eq "failed"}| sort client -Unique
