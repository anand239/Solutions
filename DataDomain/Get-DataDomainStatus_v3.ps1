...
Function Get-CleaningSchedule
{
    ...
    if(($FilesysSched_Updated -like "*Started*") -or ($FilesysSched_Updated -like "*Running*") -or ($FilesysSched_Updated -like "*Finished*"))
    {
        $signal  = "G"
        $Percent = "100 %"
    }
...
Function Get-DiskStatus
{
    ...
    try
    {
        ...
        {
            $Signal  = "R"
            $Percent = "$(($FailedDisks_Count / $TotalDisks_Count)*100) %"
        }
        else
        {
            $Signal  = "G"
    ...
    {
        $DiskStatus,$DisStatus_Signal = Get-FailedObject -HCParameter "Disk Status" -HCShortName "DS" -Message "Parse Error"
    }
    $DiskStatus,$DisStatus_Signal
}
...
if($config)
{
    ...
    if($SSHSession.connected -eq "true")
    {
        ...
        {
            $CapacityStatus,$CapacityStatus_signal  = Get-DF_hStatus -InputObject $CapacityStatus_Raw
        }
        else
        {
        ...
        if($DiskStatus_Raw)
        {
            $DiskStatus,$DisStatus_Signal = Get-DiskStatus -InputObject $DiskStatus_Raw
        }
        else
        {
            $DiskStatus,$DisStatus_Signal = Get-FailedObject -HCParameter "Disk Status" -HCShortName "DS" -Message "Failed To Run Command"
        }
        ...
        if($AlertsShow_Raw)
        {
            ...
            {
                $AlertShowStatus,$AlertsShow_Signal = Get-FailedObject -HCParameter "Data Domain Alert" -HCShortName "DS" -Message "No Alerts"
        ...
        {
            $AlertShowStatus,$AlertsShow_Signal = Get-FailedObject -HCParameter "Data Domain Alert" -HCShortName "DS" -Message "Failed To Run Command"
        }
