        ...
        $Library_Status_output = @()
            ...
            if($OsType)
            {
                ...
                if($OsType -eq "Windows")
                {
                    ...
                    ####   Free Disk Space  ####
                    $DiskspaceCommand = "Get-WmiObject win32_logicaldisk | where-object {$_.DriveType -eq 3}"
                ...
                else
                {
                    ...
                    if($failedBackup_Output)
                    {
                        ...
                        {
                            $command_IDB = "omnidb -session $($IDBBackUp.'Session Id') -media"
                            $CommandOutput_IDB = Invoke-BackupHealthCheckCommand -SshSessionId $sshsessionId -logFile $Activitylog -command $command_IDB
                            $IDB_Backup_Result = Get-IDBBackup -InputObject $CommandOutput_IDB -IDBBackUp $IDBBackUp
                            $IDBSuccess_Count = 1
                        }
                        else
                    ...
                    $Queuing30_ReportName         = $config.Reportpath +"\"+ $config.ReportType + "_" + $config.BackupApplication + "_" + $Backupdevice +"_" + "HC-Q30"  +".csv"
                    $Queuing_lt24_ReportName      = $config.Reportpath +"\"+ $config.ReportType + "_" + $config.BackupApplication + "_" + $Backupdevice +"_" + "HC-QL24" +".csv"
                    $Queuing_gt24_ReportName      = $config.Reportpath +"\"+ $config.ReportType + "_" + $config.BackupApplication + "_" + $Backupdevice +"_" + "HC-QG24" +".csv"
...
#1. IDB Backup changes
#2. Free DiskSpace command change
#3. $config.BackupType to $config.BackupApplication
#4. Name to HC_Name
#5. LibraryDeatils $sshlinesh change
