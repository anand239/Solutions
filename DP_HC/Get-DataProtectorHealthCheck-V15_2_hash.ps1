...
Function Get-DpService
{
    ...
    for($i=0;$i -lt $Service_Input.count;$i++)
    {
        ...
        $Dp_Service_Result_hash.ServiceName = $array[0].trim()
        $Dp_Service_Result_hash.ServiceStatus = $array[1].trim()
        $Dp_Service_Result += [PSCUSTOMObject] $Dp_Service_Result_hash
    ...
    $Dpservice_signal_Hash."Value"             = "$Active_Count / $Total_count"
    $Dpservice_signal_Hash.'Percentage'        = "$percent % "
    $Dpservice_signal_Hash.'Status'            = "$Signal"
    ...
    {
     $Dpservice_signal_Hash."Value"             = $_.Exception.Message
            $Dp_Service_Result_hash.ServiceName = $_.Exception.Message
           $Dp_Service_Result_hash.ServiceStatus = $null
        $Dp_Service_Result += [PSCUSTOMObject] $Dp_Service_Result_hash
    }
   [PSCUSTOMObject]$Dpservice_signal_Hash,$Dp_Service_Result
}
...
$Reportdate = ([system.datetime]::UtcNow).ToString("dd-MMM-yy HH:mm")
$date       = ([system.datetime]::UtcNow).ToString("ddMMMyy_HHmm")
