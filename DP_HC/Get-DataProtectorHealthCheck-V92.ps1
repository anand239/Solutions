...
Function Get-DpService
{
    ...
    for($i=0;$i -lt $Service_Input.count;$i++)
    {
        ...
        $obj | Add-Member NoteProperty "ProcName" $array[0].trim()
        $obj | Add-Member NoteProperty "Status [PID]" $array[1].trim()
        $Dp_Service_Result += $obj
    }
        ...
        $Library_Status_output = @()
            ...
            if($OsType)
            {
                ...
                else
                {
                    ...
                    $Queuing_lt24_signal,$Queuing_lt24_Result = Get-QueuedBackupLessThanTwentyFourHour -InputObject $QueuedBackup_Result -TotalBackup_Count $Backup_Count
                    $SignalReport += $Queuing_lt24_signal

                    
                    $Queuing_gt24_signal,$Queuing_gt24_Result = Get-QueuedBackupGreaterThanTwentyFourHour -InputObject $QueuedBackup_Result -TotalBackup_Count $Backup_Count
                    ...
                    $Library_Status_output    | Export-Csv -Path $LibraryStatus_ReportName     -NoTypeInformation
                    $Hung_Result              | Export-Csv -Path $HungBackup_ReportName        -NoTypeInformation
                    $DisabledBackupJobResult  | Export-Csv -Path $DisabledBackupJob_ReportName -NoTypeInformation
