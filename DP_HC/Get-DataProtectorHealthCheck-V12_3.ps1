...
Function Get-QueuedBackupGreaterThanTwentyFourHour
{
    ...
    $Signal_Report,$Queuing12_Result = Get-BackupSessionError -ErrorMessage "No Sessions" -HCParameter "Long Running Backup Count(>24 Hr)" -HCShortName "LB_24"
    if ("No currently running sessions." -in $InputObject)
    {
        $Signal_Report.Value = "No Sessions"
        $Signal_Report.Status = "G"
...
Function Get-Mount_Request
{
    ...
    else
    {
        ...
        {
            $Queuing24_Result.SessionId              = "No Mount Request Sessions"
            $Queuing24_Result.'Session Type'         = "No Mount Request Sessions"
            $Queuing24_Result.'Backup Specification' = "No Mount Request Sessions"
        }
...
Function Get-FreeDiskSpaceUNIX
{
    ...
    "HC_ShortName"        = "FDS_DS"
    "Value"               = "$FreeDiskSpaceDataDisk/$TotalDiskSpaceDataDisk"
    'Percentage'          = "$percent%"
    'Status'              = $Signal
    }
        ...
        $Library_Status_output = @()
            ...
            if($OsType)
            {
                ...
                if($OsType -eq "Windows")
                {
                    ...
                    #call Invoke-Command
                    if($config.ServiceHealthCheckCommand)
                    {
                    ...
                    if($config.DisabledTapeDriveCountCommand)
                    {
                    ...
                    {
                        $Disabled_TapeDrive_Output = "Disabled"
                    }

                    if($config.ScratchMediaCountCommand)
                    {
                    ...
                    {
                        $Scratch_Media_output = "Disabled"
                    }

                    if($failedBackupCommand)
                    {
