...
if ($config)
{
    ...
    if (Test-Path -Path $config.InputFile)
    {
        ...
        $DetailReportName = $config.Reportpath +"\"+$config.Account + "_" + $config.Technology + "_" + $config.backupType + "_" + "Detailed" + "_" + $Reportdate + ".txt"

        $Signalreport | Export-Csv -Path $SignalReportName -NoTypeInformation
        $DetailedReport | Out-File -Path $DetailReportName
