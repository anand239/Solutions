...
##
##Kd3HDZOFADWE8uO1
##Nc3NCtDXTlaDjofG5iZk2Un9R3wje9Warb6++K6y+s7+sjPcCddUbWxU1hnuC0+oWuYbWvsp5IFBajIsKfsI5479OtWaBYA/oKNefvGBq7M9HFXW37vX7z2dzIjNGwJ2RWDKT49sWH+PljeECy7aq7FbxSD1dv6h6N5OsFqUw3Rg9UNFHaHc3nlkk54g57fMoWGbMfmwTndrtXl4IYxb0uxkulAAJ/zemthqUPXwxzvs0j3zcts=
...
##P8HPCZWEGmaZ7/K1
##L8/UAdDXTlaDjofG5iZk2Un9R3wje9Warb6++K6y+s7+sjPcCddUbWxU1hnuC0+oWuYbWvsp5IFBajIsKfsI5479OtWaBYA/oKNefvGBq7M9HFXW37vX7z2dzIjNGwJ2RWDKT49sWH+PljeECy7aq7FbxSD1dv6h6N5OsFqUw3Rg9UNFHaHc3nlkk54g57fMoWGbMfmwTndrtXl4IYxb0uxkulAAJ/zemthqUPX063HHrHr1Zb4v
##Kc/BRM3KXhU=
##
##
...
Function Get-RemoteLibraryStatus
{
    ...
    foreach($line in $InputObject)
    {
        ...
        $output = Invoke-BackupHealthCheckCommand -SshSessionId $session.sessionId -logFile $Activitylog -command $LibRemoteCommand
        if($output)
        {
...
Function Get-DisabledJobMessage
{
    [CmdletBinding()]
    Param(
    $Message
        ...
        $Library_Status_output = @()
            ...
            if($OsType)
            {
                if($OsType -eq "Windows")
                {
                    ...
                    $StartDate = ($CurrentBackupDeviceTime).AddDays(-1).ToString("yy/MM/dd")
                    $EndDate = ($CurrentBackupDeviceTime).ToString("yy/MM/dd")
                    $failedBackupCommand  = $config.FailedBackupCountCommandWindows -replace "StartDate",$StartDate
                    $failedBackupCommand  = $failedBackupCommand -replace "EndDate",$EndDate

                    if($config.ServiceHealthCheck -eq "Enabled")
                    {
                        $Dp_Service_Output = Invoke-BackupHealthCheckCommand_Windows -ComputerName $BackupDevice -Credential $Credential -Command $config.ServiceHealthCheckCommandWindows -logFile $Activitylog
                    }
                    
                    if($config.Queuing -eq "Enabled")
                    {
                        $Backup_Output     = Invoke-BackupHealthCheckCommand_Windows -ComputerName $BackupDevice -Credential $Credential -Command $config.QueuingBackupHealthCheckCommandWindows -logFile $Activitylog
                            ...
                            try
                            {
                                if(!("No currently running sessions." -in $Backup_Result))
                                {
                                    ...
                                    foreach($line in $Hung_object)
                                    {
                                        ...
                                        $command = $config.HungBackupCountCommandWindows -replace "SessionId",$session_id
                    ...
                    {
                        $Disabled_TapeDrive_Output = Invoke-BackupHealthCheckCommand_Windows -ComputerName $BackupDevice -Credential $Credential -Command $config.DisabledTapeDriveCountCommandWindows -logFile $Activitylog
                    }

                    if($config.ScratchMediaCount -eq "Enabled")
                    {
                        $Scratch_Media_output      = Invoke-BackupHealthCheckCommand_Windows -ComputerName $BackupDevice -Credential $Credential -Command $config.ScratchMediaCountCommandWindows -logFile $Activitylog
                    }
                            ...
                            else
                            {
                                ...
                                if($IDBBackUp)
                                {
                                    ...
                                    if($IDBBackUp.Status -eq "completed")
                                    {
                                        ...
                                        $command_IDB = $config.IDBBackupCommandWindows -replace "SessionId",$IDBSessionId 
                                ...
                                {
                                    $IDB_Backup_Result = "No IDB Backup"
                                }
                            }
                        }
                        else
                    ...
                    if($config.LibraryStatus -eq "Enabled")
                    {
                        if($LocalLines)
                        {
                            Foreach($line in $LocalLines)
                            {
                                ...
                                $Command = $config.LibraryStatusLocalCommandWindows -replace "TapeLibraryName",$library_name
                    ...
                    if($config.DisabledBackupJob -eq "Enabled")
                    {
                        ...
                        $command_files = "Get-ChildItem -Recurse '$BarschedulesPath','$SchedulesPath'"
                        $FilesandFolders = Invoke-BackupHealthCheckCommand_Windows -ComputerName $BackupDevice -Credential $Credential -Command $command_files -logFile $Activitylog
                        $files = @()
                        $files = $FilesandFolders | Where-Object{-not($_.psiscontainer)}
                        if($files)
                        {
                    ...
                    if($config.Queuing -eq "Enabled")
                    {
                        if($Backup_Result)
                        {
                            ...
                            {
                                $HUNG_Output = $_.exception.message
                            }
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    {
                        $command = "uname" 
                        $UnameResult = Invoke-PlinkCommand -PlinkPath $config.plinkpath -IpAddress $BackupDevice -Credential $Credential -logFile $Activitylog -command $command -FirstTime

                        $command = "date '+%Z'"
                        $TimeZone = Invoke-PlinkCommand -PlinkPath $config.plinkpath -IpAddress $BackupDevice -Credential $Credential -logFile $Activitylog -command $command -FirstTime

                        $command = "date +'%D %T'"
                        $CurrentBackupDeviceTimeFromUnix = Invoke-PlinkCommand -PlinkPath $config.plinkpath -IpAddress $BackupDevice -Credential $Credential -logFile $Activitylog -command $command
                        $CurrentBackupDeviceTime = [datetime]$CurrentBackupDeviceTimeFromUnix
                        $StartDate = ($CurrentBackupDeviceTime).AddDays(-1).ToString("yy/MM/dd")
                        $EndDate = ($CurrentBackupDeviceTime).ToString("yy/MM/dd")
                        $failedBackupCommand  = $config.FailedBackupCountCommandUnix -replace "StartDate",$StartDate -replace "EndDate",$EndDate
                        #$failedBackupCommand  = $failedBackupCommand -replace "EndDate",$EndDate
                        
                        if($config.ServiceHealthCheck -eq "Enabled")
                        {
                            $Dp_Service_Output = Invoke-PlinkCommand -PlinkPath $config.plinkpath -IpAddress $BackupDevice -Credential $Credential -logFile $Activitylog -command $config.ServiceHealthCheckCommandUnix
                        }

                        if($config.Queuing -eq "Enabled")
                        {
                            $Backup_Output = Invoke-PlinkCommand -PlinkPath $config.plinkpath -IpAddress $BackupDevice -Credential $Credential -logFile $Activitylog -command $config.QueuingBackupHealthCheckCommandUnix
                            if($Backup_Output)
                            {
                                $Backup_Result = @(Get-BackupStatus -InputObject $Backup_Output -CurrentBackupDeviceTime $CurrentBackupDeviceTime)
                            }
                            ...
                            if($Backup_Result)
                            {
                                if(!("No currently running sessions." -in $Backup_Result))
                                {
                                    ...
                                    {
                                        $session_id = $line.sessionid
                                        $command = $config.HungBackupCountCommandUnix -replace "SessionId",$session_id
                        ...
                        if($config.DisabledTapeDriveCount -eq "Enabled")
                        {
                            $Disabled_TapeDrive_Output = Invoke-PlinkCommand -PlinkPath $config.plinkpath -IpAddress $BackupDevice -Credential $Credential -logFile $Activitylog -command $config.DisabledTapeDriveCountCommandUnix
                        }

                        if($config.ScratchMediaCount -eq "Enabled")
                        {
                            $Scratch_Media_output = Invoke-PlinkCommand -PlinkPath $config.plinkpath -IpAddress $BackupDevice -Credential $Credential -logFile $Activitylog -command $config.ScratchMediaCountCommandUnix
                        }
                            ...
                            if($FailedBackupFunctionOutput)
                            {
                                ...
                                else
                                {
                                    ...
                                    if($IDBBackUp)
                                    {
                                        ...
                                        {
                                            $IDBSessionId = $IDBBackUp.'Session Id'
                                            $command_IDB = $config.IDBBackupCommandUnix -replace "SessionId",$IDBSessionId 
                        ...
                        if($config.LibraryStatus -eq "Enabled")
                        {
                            if($LocalLines)
                            {
                                Foreach($line in $LocalLines)
                                {
                                    ...
                                    $Lnput_Lib = $line -split ','
                                    $library_name = $Lnput_Lib[1].trim()
                                    $Command = $config.LibraryStatusLocalCommandUnix -replace "TapeLibraryName",$library_name
                        ...
                        if($config.DisabledBackupJob -eq "Enabled")
                        {
                            ...
                            if($Files -ne $null)
                            {
                                ...
                                Foreach($file in $files)
                                {
                                    ...
                                    $obj = New-Object psobject
                                    $obj | Add-Member NoteProperty "Specification" "$basename"
                                    $obj | Add-Member NoteProperty "Status" "Failed to open File"
                                    $DisabledBackupJobResult += $obj
                    ...
                    else
                    {
                        ...
                        {
                        $command = "date '+%Z'"
                        $TimeZone = Invoke-BackupHealthCheckCommand -SshSessionId $sshsessionId.sessionId -logFile $Activitylog -command $command
                        ...
                        if($config.ServiceHealthCheck -eq "Enabled")
                        {
                            $Dp_Service_Output = Invoke-BackupHealthCheckCommand -SshSessionId $sshsessionId.sessionId -logFile $Activitylog -command $config.ServiceHealthCheckCommandUnix
                        }
                        
                        if($config.Queuing -eq "Enabled")
                        {
                            $Backup_Output = Invoke-BackupHealthCheckCommand -SshSessionId $sshsessionId.sessionId -logFile $Activitylog -command $config.QueuingBackupHealthCheckCommandUnix
                        ...
                        if($config.DisabledTapeDriveCount -eq "Enabled")
                        {
                            $Disabled_TapeDrive_Output = Invoke-BackupHealthCheckCommand -SshSessionId $sshsessionId.sessionId -logFile $Activitylog -command $config.DisabledTapeDriveCountCommandUnix
                        }

                        if($config.ScratchMediaCount -eq "Enabled")
                        {
                            $Scratch_Media_output = Invoke-BackupHealthCheckCommand -SshSessionId $sshsessionId.sessionId -logFile $Activitylog -command $config.ScratchMediaCountCommandUnix
                        }
                        ...
                        else
                        {
                            ...
                            $Dpservice_signal,$Dp_Service_Result                        = Get-DpServiceMessage         -Message "Failed to connect to server"
                            $Queuing_gt30_signal,$Queuing_30_Result                     = Get-BackupSessionMessage     -Message "Failed to connect to server" -HCParameter "Queuing Backup Count(>30 min)" -HCShortName "WQB"
                            $Queuing_lt24_signal,$Queuing_lt24_Result                   = Get-BackupSessionMessage     -Message "Failed to connect to server" -HCParameter "Long Running Backup Count(>12 Hr and <24 Hr)" -HCShortName "LB_12"
                            $Queuing_gt24_signal,$Queuing_gt24_Result                   = Get-BackupSessionMessage     -Message "Failed to connect to server" -HCParameter "Long Running Backup Count(>24 Hr)" -HCShortName "LB_24"
                            $Mount_req_signal,$Mount_Request_Result                     = Get-BackupSessionMessage     -Message "Failed to connect to server" -HCParameter "Mount Request" -HCShortName "MR"
                            $Disabled_TapeDrive_signal,$Disabled_TapeDrive_Result       = Get-DisabledTapeDriveMessage -Message "Failed to connect to server"
                            $Scratch_Media_signal,$Scratch_Media_Result                 = Get-ScratchMediaMessage      -Message "Failed to connect to server"
                            $Failed_bck_signal,$Failed_Bck_result                       = Get-FailedBackupMessage      -Message "Failed to connect to server" -HCParameter "Failed Backup Count"    -HCShortName "FB"
                            $Critical_Backup_signal,$Critical_Bck_result                = Get-FailedBackupMessage      -Message "Failed to connect to server" -HCParameter "Critical Backup Status" -HCShortName "CB"
                            $IDBBackup_Signal,$IDB_Backup_Result                        = Get-IDBMessage               -Message "Failed to connect to server"
                            $Hung_Bck_signal,$HungBackup_Result                         = Get-BackupSessionMessage     -Message "Failed to connect to server" -HCParameter "Hung Backup Count" -HCShortName "HB"
                            $DisabledBackupJob_Signal,$DisabledBackupJob_Result         = Get-DisabledJobMessage       -Message "Failed to connect to server"
                            $FreeDiskSpace_signal,$FreeDiskSpace_Result                 = Get-FreeDiskSpaceMessage     -Message "Failed to connect to server" -HCParameter "Free Disk Space" -HCShortName "FDS"
                            $FreeDiskSpaceDataDisk_signal,$FreeDiskSpaceDataDisk_Result = Get-FreeDiskSpaceMessage     -Message "Failed to connect to server" -HCParameter "Free Disk Space Data Disk" -HCShortName "FDS_DS"
                            $Library_Status_signal,$LibraryStatus_Result                = Get-LibraryMessage           -Message "Failed to connect to server"
                    ...
                    if($config.ServiceHealthCheck -eq "Enabled")
                    {
                        ...
                        {
                            $Dpservice_signal,$Dp_Service_Result = Get-DpServiceMessage -Message "Failed To Run Command"
                        }
                    }
                    else
                    {
                    ...
                    if($config.Queuing -eq "Enabled")
                    {
                        ...
                        {
                            $Queuing_gt30_signal,$Queuing_30_Result   = Get-BackupSessionMessage -Message "Failed To Run Command" -HCParameter "Queuing Backup Count(>30 min)" -HCShortName "WQB"
                            $Queuing_lt24_signal,$Queuing_lt24_Result = Get-BackupSessionMessage -Message "Failed To Run Command" -HCParameter "Long Running Backup Count(>12 Hr and <24 Hr)" -HCShortName "LB_12"
                            $Queuing_gt24_signal,$Queuing_gt24_Result = Get-BackupSessionMessage -Message "Failed To Run Command" -HCParameter "Long Running Backup Count(>24 Hr)" -HCShortName "LB_24"
                            $Mount_req_signal,$Mount_Request_Result   = Get-BackupSessionMessage -Message "Failed To Run Command" -HCParameter "Mount Request" -HCShortName "MR"
                        }
                    }
                    else
                    {
                    ...
                    if($config.DisabledTapeDriveCount -eq "Enabled")
                    {
                        ...
                        {
                            $Disabled_TapeDrive_signal,$Disabled_TapeDrive_Result = Get-DisabledTapeDriveMessage -Message "Failed To Run Command"
                        }
                    }
                    else
                    {
                    ...
                    if($config.FailedBackupCount -eq "Enabled")
                    {
                        ...
                        {
                            $IDBBackup_Signal,$IDB_Backup_Result = Get-IDBMessage -Message "Failed To Run Command"
                        }
                        else
                        {
                            if("No Sessions matching" -in $IDB_Backup_Result)
                            {
                                $IDBBackup_Signal,$IDB_Backup_Result = Get-IDBMessage -Message "No Sessions Matching"
                                $IDBBackup_Signal.Status = "G"
                            }
                            elseif("No IDB Backup" -in $IDB_Backup_Result)
                            {
                                $IDBBackup_Signal,$IDB_Backup_Result = Get-IDBMessage -Message "No IDB Backup"
                                $IDBBackup_Signal.Status = "R"
                            }
                            else
                            {
                                $TotalIDB_Count = 1
                    ...
                    {
                            $IDBBackup_Signal,$IDB_Backup_Result = Get-IDBMessage -Message "Disabled"
                    ...
                    if($config.LibraryStatus -eq "Enabled")
                    {
                        ...
                        if(!($sshLines -or $locallines))
                        {
                            $Library_Status_signal,$LibraryStatus_Result = Get-LibraryMessage -Message "Invalid Librarydetails.txt"
                        }
                        else
                        {
                            if($Library_Status_output)
                            {
                                ...
                                $LibraryStatus_Result = AddMember -InputObject $Library_Status_output -HCParamater "Library Status" | select `
                                                        Technology,ReportType,
                                                        Account,BackupServer,
                                                        ReportDate,HC_Parameter,
                                                        'Library Name/IP','Status'
                                $Library_Status_signal = [PSCUSTOMObject] @{
                                "Technology"           = $config.Technology
                                "ReportType"           = $config.ReportType
                                "BackupApplication"    = $config.BackupApplication
                            ...
                            {
                                $Library_Status_signal,$LibraryStatus_Result = Get-LibraryMessage -Message "Failed To Run Command"
                            }
                        }
                    }
                    else
                    {
                            $Library_Status_signal,$LibraryStatus_Result = Get-LibraryMessage -Message "Disabled"
                    ...
                    if($config.Queuing -eq "Enabled")
                    {
                        ...
                        {
                            $Hung_Bck_signal,$HungBackup_Result = Get-BackupSessionMessage -Message "Failed To Run Command" -HCParameter "Hung Backup Count" -HCShortName "HB"
                        }
                        else
                        {
                            try
                            {
                                ...
                                {
                                    $Hung_Bck_signal,$HungBackup_Result = Get-BackupSessionMessage -Message "No Sessions" -HCParameter "Hung Backup Count" -HCShortName "HB"
                                    $Hung_Bck_signal.status = "G"
                                }
                                else
                                {
                                    if($Backup_Result.GetType().name -eq "String")
                                    {
                                        $Total_Bck_count_hung = 0
                                    }
                                    else
                                    {
                                    ...
                                    if($Hung_Result)
                                    {
                                        $HungBackup_Result = AddMember -InputObject $Hung_Result -HCParamater "Hung Backup Count" | select `
                                                                Technology,ReportType,
                                                                Account,BackupServer,
                                                                ReportDate,HC_Parameter,
                                                                'SessionID','Backup Specification','Session data size [kB]'
                                    }
                                    else
                                    {
                            ...
                            {
                                $Hung_Bck_signal,$HungBackup_Result = Get-BackupSessionMessage -Message "Parsing Error" -HCParameter "Hung Backup Count" -HCShortName "HB"
                                $Hung_Bck_signal,$HungBackup_Result
                            }
                        }
                    }
                    else
                    {
                        $Hung_Bck_signal,$HungBackup_Result = Get-BackupSessionMessage -Message "Disabled" -HCParameter "Hung Backup Count" -HCShortName "HB"
                    ...
                    if($config.DisabledBackupJob -eq "Enabled")
                    {
                        ...
                        {
                            $DisabledBackupJob_Signal,$DisabledBackupJob_Result = Get-DisabledJobMessage -Message "Failed to Run Command"
                        }
                        else
                        {
                            $TotalBackupCount_Disabled  = $files.Count
                            $DisabledBackup_Count = ($DisabledBackupJobResult|Where-Object{$_.status -eq "Disable"}).Count
                            ...
                            {
                                $DisabledBackupJob_Result = AddMember -InputObject $DisabledBackupJobResult -HCParamater "Disabled Backup Job Count" | select `
                                                            Technology,ReportType,
                                                            Account,BackupServer,
                                                            ReportDate,HC_Parameter,
                    ...
                    {
                            $DisabledBackupJob_Signal,$DisabledBackupJob_Result = Get-DisabledJobMessage -Message "Disabled"
...
#changes
#1. Added Check-Access Function.
#2. Added Verification for linux connnection.
#3. RemoveFiles function added.