...
if($config)
{
    ...
    if(Test-Path -Path $config.Serversfile)
    {
        $Servers = Get-Content $config.Serversfile | where{$_}
    }
    else
    {
        Write-Host "Invalid $($config.Serversfile)!" -ForegroundColor Red
        exit
    }
    ...
    if($Servers)
    {
        foreach($server in $servers)
        {
            ...
            catch
            {
                ...
                $DDCapacity        += [pscustomobject] @{
                Server              = $server
                IPv4Hostname        = "Invalid Credential file"
                TotalCapacity       = "Invalid Credential file"
                ServerUtilization   = "Invalid Credential file"
                FileSystemAvailable = "Invalid Credential file"
                FileSystemUsed      = "Invalid Credential file"
                }
                break
            }
            ...
            if($SSHSession.connected -eq "True")
            {
                ...
                if($DDCapacityOutput_raw)
                {
                    ...
                    if($DDCapacityOutput)
                    {
                        foreach($line in $DDCapacityOutput)
                        {
                            ...
                            $DDCapacity        += [pscustomobject] @{
                            Server              = $server
                            IPv4Hostname        = ($values[0] -split "\s")[0]
                            TotalCapacity       = $values[1]
                            ServerUtilization   = $values[2]
                            FileSystemAvailable = $values[4]
                            FileSystemUsed      = $values[5]
                            }
                        }
                    }
                    else
                    {
                        $DDCapacity        += [pscustomobject] @{
                        Server              = $server
                        IPv4Hostname        = "No DataDomain Available"
                        TotalCapacity       = "No DataDomain Available"
                        ServerUtilization   = "No DataDomain Available"
                        FileSystemAvailable = "No DataDomain Available"
                        FileSystemUsed      = "No DataDomain Available"
                        }
                    }
                }
                else
                {
                    $DDCapacity        += [pscustomobject] @{
                    Server              = $server
                    IPv4Hostname        = "Failed to get data"
                    TotalCapacity       = "Failed to get data"
                    ServerUtilization   = "Failed to get data"
                    FileSystemAvailable = "Failed to get data"
                    FileSystemUsed      = "Failed to get data"
                    }
                }
            }
            else
            {
                ...
                $DDCapacity        += [pscustomobject] @{
                Server              = $server
                IPv4Hostname        = "Failed to connect to server"
                TotalCapacity       = "Failed to connect to server"
                ServerUtilization   = "Failed to connect to server"
                FileSystemAvailable = "Failed to connect to server"
                FileSystemUsed      = "Failed to connect to server"
                }
            }
        }
