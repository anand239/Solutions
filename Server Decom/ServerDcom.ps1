    ...
    if($session.connected -eq "true")
    {
        ...
        if($hostnames)
        {
            ...
            foreach($Line in $hostnames)
            {
                ...
                $status = Invoke-ServerDecomCommand -SshSessionId $sshsessionId.sessionId -logFile $Activitylog -command "sudo systemctl status zabbix-agent"
                if($status -notlike "disabled")
                {
                    Write-Log -Path $Activitylog -Entry "unable to stop zabix-agent service, exiting for $hostname!" -Type Information -ShowOnConsole
                    break
                }
