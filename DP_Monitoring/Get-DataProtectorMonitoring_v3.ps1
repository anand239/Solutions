                ...
                if($SessionIdObjectReport)
                {
                    ...
                    else
                    {
                        foreach($object in $SessionIdObjectReport)
                        {
                            $SessionIdObjectReport_Split = $object -split "\s" | where{$_}
                            $Host_Name  = ($Object -split ":")[0]
                            $MountPoint = $SessionIdObjectReport_Split[0] -split ":",2
                            $BkpType    = $SessionIdObjectReport_Split[1]
                            $ObjectStatus = $SessionIdObjectReport_Split[2]
                    ...
                    if($SessionIdObjectReport)
                    {
                        ...
                        if($FailedObject)
                        {
                            ...
                            else
                            {
                                ...
                                $completed_Client += Get-SessionIdObject -Specification $Failed_Specification -SessionId $Failed_SessionId -Mode $Failed_Mode -BackupType $BkpType -Client "NA" -MountPoint "NA" -ObjectStatus $Failed_Session.status -EndTime $End_Time
                            }
                        }
