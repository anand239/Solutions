...
$DP_Mon_Detail_result = @()
foreach($line in $DP_Mon_Detail)
{
    $ThirtFivedays = [datetime]$line.'End time'
    $timespan = (New-TimeSpan -Start $ThirtFivedays -End $today).TotalDays
    if(-not($timespan -gt 35))
    {
        if($ThirtFivedays -lt $yesterday)
        {
            $DP_Mon_Detail_result += $line
            $DP_Mon_Detail_result | Add-Member NoteProperty "Validation" "Skip"
        }
    }
}