...
Function Get-UpdatedPriorityFile
{
    ...
    if($SSHSession.connected -eq "True")
    {
        ...
        #$command = "cat " + $config.Configpath + "/" + $config.configfilename
        $InputData = Invoke-NonWindowsCommand -SshSessionId $sshsession.sessionId -logFile $Activitylog -command $command
        if($InputData)
        {
            $TrimmedData = $InputData | ForEach-Object{$_.trim()}
            foreach($line in $TrimmedData)
            {
            ...
            if($FilteredLines)
            {
                ...
                if($UserInput -eq "Y")
                {
                    ...
                    $ServerBackupMonPath = $config.Configpath + "/" + $config.configfilename
                    $Pri_new = $array | ForEach-Object{$_ + "\n"}
                    $CopyCommand = "echo -e `"$Pri_new`" > $ServerBackupMonPath"
                    ...
                    $CopyData = Invoke-NonWindowsCommand -SshSessionId $sshsession.sessionId -logFile $Activitylog -command $CopyCommand
                }
                else
                {
