...
.DESCRIPTION
  Compares two csv files and updates the data in one.

.INPUTS
  Configfile - config.json
  InputCSV   - DPA_Manual.csv,DPA_Mail.csv
   
.NOTES
  Script:         Get-DPAReport.ps1
  ...
  .History:
        ...
        1.0     22/12/2021      Chintalapudi Anand Vardhan   Initial Release
.EXAMPLE
  Script Usage 
...
if($config)
{
    ...
    foreach($sheet in $MailSheets)
    {
        ...
        Write-Host "Fetching data from $(Split-Path $sheet -Leaf)" -ForegroundColor Green
