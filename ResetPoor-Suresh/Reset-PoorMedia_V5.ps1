...
if($config)
{
    ...
    $FailedtoRunReportName = $config.ReportPath + "\" + $config.Account + "_" + $($config.BackupServer) + "_" + "FailedtoRun"    + "_" + $ReportDate + ".txt"
    ...
    if($OSType)
    {
                ...
                foreach($Pool in $Pools)
                {
                    ...
                    if($PoorPools)
                    {
                        ...
                        foreach($PoorPool in $PoorPools)
                        {
                            ...
                            if($PoorLabelSplit[2].Length -eq 8)
                            {
                            ...
                            {
                                if($PoorLabelSplit[1].Length -eq 8)
                                {
                                    $PoorLabel   = $PoorLabelSplit[1]
                                    $PoorLabels += $PoorLabel
                                }
                                elseif($PoorLabelSplit[1].Length -eq 10)
                                {
                                    $PoorLabel   =  $PoorLabelSplit[1].Substring(1,8)
                                    $PoorLabels += $PoorLabel
                                }
                                else
                                {
                    ...
                    else
                    {
                        ...
                        $PoorMediaReport    += [PsCustomobject] @{
                        "Date"               = ([system.datetime]::UtcNow).Addhours(-5).ToString("dd-MM-yyyy hh:mm")
        ...
        else
        {
            ...
            $FailedtoRunReportName = $config.ReportPath + "\" + $config.Account + "_" + $BackupDevice[1] + "_" + "FailedtoRun"    + "_" + $ReportDate + ".txt"
    ...
    if($FailedtoRun)
    {
        $FailedtoRun | Out-File $FailedtoRunReportName
    }
    if($config.SendEmail -eq "Yes")
    {
        ...
        if($FailedtoRun)
        {
            $attachment += $FailedtoRunReportName
        }
