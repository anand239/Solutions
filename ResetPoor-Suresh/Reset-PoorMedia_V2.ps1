...
if($config)
{
    ...
    {
        $Pools = Get-Content $config.InputPoolFile | where{$_}
    }
    else
    {
    ...
    if($OSType)
    {
        if($OSType -eq "Windows")
        {
            ...
            {
                $ListPoolCommand = $config.Command1Windows -replace "PoolName","$Pool"
        ...
        else
        {
            ...
            if($Session.connected -eq "True")
            {
                foreach($Pool in $Pools)
                {
                    $ListPoolCommand = $config.Command1NonWindows -replace "PoolName","$Pool"
