...
Function Get-Colour
{
    ...
    {
        "#FF4D4D" { $colour = "Red" }
        "#FFFF66" { $colour = "Yellow" }
        "#FF9900" { $colour = "Orange" }
        "#66FF66" { $colour = "Green" }
        default { $colour}
    }
    $colour
...
if($config)
{
    ...
    if($Reports)
    {
        ...
        foreach($Report in $Reports)
        {
            ...
            $MailDate = ($Report.ReceivedTime).tostring("yyyy-MM-dd")
            ...
            foreach($rowMatch in $rowMatches)
            {
                ...
                $Colourcode = $colormatch.Groups[1].Value
                $colour = Get-Colour -InputColourCode $Colourcode
                ...
                $InstancesLink   =  ($rowCells[4].Value -split "`"")[3]
        ...
        if($Color -eq "All")
        {
            $tabledata | Export-Csv "SRMVersion.csv" -NoTypeInformation
        }
        else
        {
            $tabledata | where{$_.color -eq $Color} | Export-Csv "SRMVersion.csv" -NoTypeInformation
        }
    }
    else
    {
