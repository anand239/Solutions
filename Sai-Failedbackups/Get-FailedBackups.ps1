...
if($config)
{
    ...
    if($Servers)
    {
        foreach($Server in $Servers)
        {
            ...
            $ListofSessionsCommand = $config.ListofSessionsCommand -replace "Servername",$server #-replace "Startdate",$StartDate -replace "Enddate",$EndDate
            ...
            $FailedSessions = $ListOfSessions | where{(($_.status -eq "Failed") -or ($_.status -eq "Completed\Failures"))}
            if($FailedSessions)
            {
                foreach($failedSession in $FailedSessions)
                {
                    $Failed_SessionId = $failedSession.'session id'
                    $Failed_SessionLogCommand = $config.SessionLogCommand -replace "Sessionid","$Failed_SessionId"
                    ...
                    if($Failed_SessionLog)
                    {
                        ...
                        if($Critical_Major)
                        {
                            $Error_Log = $Critical_Major.Split([Environment]::NewLine)|where{$_} |select -Skip 1| select -First 1
